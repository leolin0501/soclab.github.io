...
(
    ...
    always@(*)begin
        if(read_configuration) rdata = {configuration[31:4],ss_tready,configuration[2:0]};
    ...
    always@(posedge axis_clk or negedge axis_rst_n)begin
        if(!axis_rst_n)begin
            ...
            else if(shift_counter == 0 && data_write_en) configuration <= configuration | 32'h0000_0010;
            else if(sm_tready && sm_tvalid && sm_tlast) configuration <= 32'h0000_0006;
            else if(rready && rvalid && read_configuration) configuration <= configuration & 32'hFFFF_FFED;
